...
> One of the most important techniques for achieving deep modules is *information hiding*. This technique was first described in a classic paper by David Parnas [1]. The basic idea is that each module should encapsulate a few pieces of knowledge, which represent design decisions. The knowledge is embedded in the module’s implementation but does not appear in its interface, so it is not visible to other modules.
