...
> One of the most important techniques for achieving deep modules is *information hiding*. This technique was first described in a classic paper by David Parnas <sup>1</sup>. The basic idea is that each module should encapsulate a few pieces of knowledge, which represent design decisions. The knowledge is embedded in the module’s implementation but does not appear in its interface, so it is not visible to other modules.

实现深模块最重要的技术之一是 *信息隐藏*。该想法最早是由 David Parnas 在一篇经典的论文 <sup>1</sup> 中提出的。基本思想是每个模块应封装一些知识，这些知识代表设计决策。该知识嵌入在模块的实现中，但不会出现在其接口中，因此其他模块不可见。
